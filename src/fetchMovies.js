    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            const organizeByAllCriteria = (allGenreMovies) => {
                ...
                console.log("DEE", organizedGenres)
...
//     const API_KEY = process.env.REACT_APP_API_KEY || 'YOUR_API_KEY'; // Access the environment variable or provide a fallback
...
//             // Function to organize movies in each genre by all three criteria
//             const organizeByAllCriteria = (allGenreMovies) => {
//                 const organizedGenres = {};
//                 for (const genre in allGenreMovies) {
//                     const sortedByPopularity = sortByProperty(allGenreMovies[genre], 'popularity');
//                     const sortedByVoteCount = sortByProperty(allGenreMovies[genre], 'voteCount');
//                     const sortedByVoteAverage = sortByProperty(allGenreMovies[genre], 'voteAverage');
//                     // Sort by revenue in descending order
//                     // const sortedByRevenue = allGenreMovies[genre].sort((a, b) => b.popularity - a.popularity);

//                     // Reverse the order and assign ranks directly from the fetched list (sorted by revenue)
//                     const byRevenue = allGenreMovies[genre].slice().reverse().map((movie, index) => ({
//                         ...movie,
//                         rank: index + 1 // Rank starting from 1 to length of the list
//                     }));

//                     organizedGenres[genre] = {
//                         byPopularity: withRankProperty(sortedByPopularity),
//                         byVoteCount: withRankProperty(sortedByVoteCount),
//                         byVoteAverage: withRankProperty(sortedByVoteAverage),
//                         byRevenue: withRankProperty(byRevenue), // Add the revenue-sorted list directly
//                     };
//                 }
//                 return organizedGenres;
//             };
...
//             const organizedGenres = organizeByAllCriteria(allGenreMovies);
//             console.log("PETER STOop HENDER", organizedGenres);
//             setMovies(organizedGenres);
//         };
